      ...
      .subscribe(() => {
        ...
        error => {
          if (error.error.error === '同名学期已存在') {
            this.sweetAlertService.showError('编辑失败', '同名学期已存在', '');
          } else if (error.error.error === '相同开始时间的学期已存在') {
            this.sweetAlertService.showError('编辑失败', '相同开始时间的学期已存在', '');
          } else if (error.error.error === '相同结束时间的学期已存在') {
            this.sweetAlertService.showError('编辑失败', '相同结束时间的学期已存在', '');
          } else if (error.error.error === '相似时间的学期已存在') {
            this.sweetAlertService.showError('编辑失败', '相似时间的学期已存在', '');
          } else if (error.error.error === '学期时间设置异常') {
            this.sweetAlertService.showError('编辑失败', '学期时间设置异常', '');
          } else if (error.error.error === '学期的周数不得少于16周') {
            this.sweetAlertService.showError('编辑失败', '学期的周数不得少于16周', '');
          } else if (error.error.error === '学期的周数不得多于22周') {
            this.sweetAlertService.showError('编辑失败', '学期的周数不得多于22周', '');
